...
from CodernityDB3.index import Index, IndexException, DocIdNotFound, ElemNotFound
...
# twolvl_cache(self._read_single_leaf_record)
    ...
    def create_index(self):
        ...
        self.buckets = open(os.path.join(self.db_path, self.name +
...
#                    should't happen, crashes earlier on id index
    ...
    def _update_element(self, leaf_start, key_index, new_data):
        self.buckets.seek(self._calculate_key_position(leaf_start, key_index, 'l') + self.key_size)
        self.buckets.write(struct.pack(b'<' + self.meta_format.encode(), *new_data))

#        self._read_single_leaf_record.delete(leaf_start_position, key_index)

    def _delete_element(self, leaf_start, key_index):
        self.buckets.seek(self._calculate_key_position(leaf_start, key_index, 'l') + self.single_leaf_record_size - 1)
...
#        self._read_single_leaf_record.delete(leaf_start_position, key_index)
    ...
    def _leaf_linear_key_search(self, key, start, start_index, end_index):
        self.buckets.seek(start)
        data = self.buckets.read((end_index - start_index + 1) * self.single_leaf_record_size)
        curr_key = struct.unpack(b'<' + self.key_format.encode(), data[:self.key_size])[0]
    ...
    def _node_linear_key_search(self, key, start, start_index, end_index):
        ...
        while curr_key != key:
            curr_index += 1
            curr_key = struct.unpack('<' + self.key_format, data[:self.key_size])[0]
            data = data[self.key_size + self.pointer_size:]
        return start_index + curr_index
        ...
        if create_new_root:  # leaf is a root
            ...
            if nr_of_records_to_rewrite > half_size:  # insert key into first half of leaf
                ...
                if self._update_if_has_deleted(leaf_start,
                    ...
                    return None
                key_moved_to_parent_node = records_to_rewrite[-new_leaf_size * 5]
