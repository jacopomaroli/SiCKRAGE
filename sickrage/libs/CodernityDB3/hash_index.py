...
class IU_HashIndex(Index):
    ...
    def __init__(self, db_path, name, entry_line_format='<32s{key}IIcI', hash_lim=0xfffff, storage_class=None,
                 key_format='c'):
        """
        ...
        self.data_start = (self.hash_lim + 1) * self.bucket_line_size + self._start_ind + 2
    ...
    def open_index(self):
        ...
        self.buckets = open(os.path.join(self.db_path, self.name + "_buck"), 'r+b', buffering=0)
        self._fix_params()
        self._open_storage()
    ...
    def create_index(self):
        ...
        with open(os.path.join(self.db_path, self.name + "_buck"), 'w+b') as f:
    ...
    def _find_key(self, key):
        ...
        if curr_data:
            ...
            if status == b'd':  # when first record from many is deleted
                while True:
                    found_at, doc_id, l_key, start, size, status, _next = self._locate_key(key, _next)
                    if status != b'd':
                        break
    ...
    def _calculate_position(self, key):
        ...
        return abs(int(hashlib.sha1(key).hexdigest(), 16) % 10 ** 8 & self.hash_lim) * self.bucket_line_size + self._start_ind
    ...
    def _locate_key(self, key, start):
        ...
        while True:
            ...
            try:
                ...
                raise ElemNotFound("Not found")  # not found but might be also broken
            if l_key == key:
                break
            else:
                if not _next:
    ...
    def update(self, doc_id, key, u_start=0, u_size=0, u_status=b'o'):
        ...
        self.buckets.write(self.entry_struct.pack(doc_id.encode() if isinstance(doc_id, str) else doc_id,
                                                  key.encode() if isinstance(key, str) else key,
                                                  u_start,
                                                  u_size,
                                                  u_status.encode() if isinstance(u_status, str) else u_status,
                                                  _next))
        self.flush()
        ...
        if location:
            ...
            try:
                ...
                self.buckets.write(self.entry_struct.pack(doc_id.encode() if isinstance(doc_id, str) else doc_id,
                                                          ...
                                                          start,
                                                          size,
                                                          status.encode() if isinstance(status, str) else status,
                                                          _next))
                #                self.flush()
                self.buckets.seek(found_at)
                self.buckets.write(self.entry_struct.pack(_doc_id.encode() if isinstance(_doc_id, str) else _doc_id,
                                                          _key.encode() if isinstance(_key, str) else _key,
                                                          _start,
                                                          _size,
                                                          _status.encode() if isinstance(_status, str) else _status,
                                                          wrote_at))
            else:
                self.buckets.seek(found_at)
                self.buckets.write(self.entry_struct.pack(doc_id.encode() if isinstance(doc_id, str) else doc_id,
    ...
    def _fix_link(self, key, pos_prev, pos_next):
        ...
        if pos_prev >= self.data_start:
            ...
            if data:
                ...
                self.buckets.seek(pos_prev)
                self.buckets.write(self.entry_struct.pack(doc_id.encode() if isinstance(doc_id, str) else doc_id,
                                                          l_key.encode() if isinstance(l_key, str) else l_key,
                                                          start,
                                                          size,
                                                          status.encode() if isinstance(status, str) else status,
                                                          pos_next))
...
class IU_UniqueHashIndex(IU_HashIndex):
    ...
    def update(self, key, rev, u_start=0, u_size=0, u_status=b'o'):
        start_position = self._calculate_position(key)
        self.buckets.seek(start_position)
        curr_data = self.buckets.read(self.bucket_line_size)
        # test if it's unique or not really unique hash
        ...
        if u_size == 0:
            u_size = size
        self.buckets.seek(found_at)
        self.buckets.write(self.entry_struct.pack(key.encode() if isinstance(key, str) else key,
                                                  rev.encode() if isinstance(rev, str) else rev,
                                                  u_start,
                                                  u_size,
                                                  u_status.encode() if isinstance(u_status, str) else u_status,
                                                  _next))
    ...
    def insert(self, key, rev, start, size, status=b'o'):
        start_position = self._calculate_position(key)
        self.buckets.seek(start_position)
        curr_data = self.buckets.read(self.bucket_line_size)
        ...
        if location:
            # last key with that hash
            found_at, _key, _rev, _start, _size, _status, _next = self._find_place(location, key)
            self.buckets.seek(0, 2)
            wrote_at = self.buckets.tell()
            ...
            #            self.flush()
            self.buckets.seek(found_at)
            self.buckets.write(self.entry_struct.pack(_key.encode() if isinstance(_key, str) else _key,
                                                      _rev.encode() if isinstance(_rev, str) else _rev,
                                                      _start,
                                                      _size,
                                                      _status.encode() if isinstance(_status, str) else _status,
                                                      wrote_at))
    ...
    def make_key_value(self, data):
        ...
        if len(_id) != 32:
            raise IndexPreconditionsException("Invalid _id length")
        del data['_id']
        del data['_rev']
        return _id, data
