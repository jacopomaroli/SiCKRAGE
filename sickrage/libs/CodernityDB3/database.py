...
class Database(object):
    ...
    def _add_single_index(self, p, i, index):
        ...
        with io.FileIO(os.path.join(p, "%.2d%s" % (i, index.name) + '.py'), 'w') as f:
            f.write(bytes(header_for_indexes(index.name,
    ...
    def _read_index_single(self, p, ind, ind_kwargs={}):
        ...
        with io.FileIO(os.path.join(p, ind), 'r') as f:
            name = f.readline()[2:].strip().decode()
            _class = f.readline()[2:].strip().decode()
            code = f.read().decode()
        try:
    ...
    def __write_index(self, new_index, number=0, edit=False, ind_kwargs=None):
        ...
        if isinstance(new_index, str) and not new_index.startswith("path:"):
            ...
            if name in self.indexes_names and not edit:
                raise IndexConflict("Already exists")
            if edit:
                previous_index = list(filter(lambda x: x.endswith('.py') and x[2:-3] == name, os.listdir(p)))
            ...
            ind = new_index
            init_arguments = new_index.__class__.__init__.__code__.co_varnames[3:]  # ignore self, path and name
            ...
            if edit:
                # code duplication...
                previous_index = list(filter(lambda x: x.endswith('.py') and x[2:-3] == ind.name, os.listdir(p)))
                if not previous_index:
                    raise PreconditionsException(
                        "Can't edit index that's not yet in database")
                number = int(previous_index[0][:2])
...
#        storage = self.storage
    ...
    def _single_reindex_index(self, index, data):
        doc_id, rev, start, size, status = self.id_ind.get(data['_id'])  # it's cached so it's ok
...
#                l_key, start, size, status = next(gen)
    ...
    def get_db_details(self):
        ...
        """
        props = {'path': self.path, 'size': self.__get_size(), 'indexes': self.indexes_names.keys(),
                 'cdb_environment': cdb_environment}
        return props
